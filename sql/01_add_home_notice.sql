...
INSERT INTO `home_notice` (`notice_type`, `title`, `summary`, `target_url`, `cover_url`, `is_pinned`, `sort_order`, `status`)
SELECT * FROM (
  SELECT 1, '平台规则更新说明', '报价前请先完善交付范围、阶段目标与验收方式，减少来回确认成本。', '/publish', NULL, 0, 10, 1
  UNION ALL
  SELECT 2, '五月需求撮合活动', '完成实名认证和技能审核的开发者，可优先展示在活动推荐位。', '/market', NULL, 1, 20, 1
  UNION ALL
  SELECT 1, '管理员可维护首页内容', '轮播图与右侧公告活动都支持在后台独立新增、编辑、上下线。', '/admin/banners', NULL, 0, 30, 1
) AS seed (`notice_type`, `title`, `summary`, `target_url`, `cover_url`, `is_pinned`, `sort_order`, `status`)
WHERE NOT EXISTS (
