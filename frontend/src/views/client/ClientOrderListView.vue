  ...
  <section class="page-card">
    ...
    <el-table :data="orders" v-loading="loading">
      ...
      <el-table-column label="操作" width="440" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-if="row.status === 1" size="small" type="primary" @click="handlePay(row.id)">托管支付</el-button>
          <el-button v-if="row.status === 4" size="small" type="success" @click="handleAccept(row.id)">验收通过</el-button>
          <el-button v-if="row.status === 4" size="small" type="danger" @click="handleReject(row.id)">打回修改</el-button>
    ...
    <el-drawer v-model="detailVisible" size="40%" title="订单详情">
      <div v-if="currentOrder" class="detail-grid">
        ...
        <p><strong>开发者 ID：</strong>{{ currentOrder.developerId }}</p>
...
async function showDetail(orderId) {
  try {
    const response = await fetchClientOrderDetail(orderId)
...
async function handlePay(orderId) {
  try {
    ...
    ElMessage.success('托管支付成功')
    loadOrders()
  } catch (error) {
    ElMessage.error(error.message || '托管支付失败')
  }
}

async function handleAccept(orderId) {
  try {
    await acceptOrder(orderId, '用户验收通过')
    ElMessage.success('当前阶段验收通过')
    loadOrders()
  } catch (error) {
...
async function handleReject(orderId) {
  try {
    const result = await ElMessageBox.prompt('请填写打回原因', '打回修改', {
      inputValue: '需要继续修改'
    })
    await rejectOrder(orderId, result.value)
    ElMessage.success('已打回开发者继续修改')
    loadOrders()
  } catch (error) {
    if (error !== 'cancel') {
...
function openDispute(orderId) {
  router.push(`/client/disputes?orderId=${orderId}`)
}
